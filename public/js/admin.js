...
var chartExists = function chartExists(cardChart) {
  var exists = false;
  ...
  custom: function custom(tooltipModel) {
    ...
    if (tooltipModel.body) {
      ...
      bodyLines.forEach(function (body, i) {
        ...
        var span = ''; // Add color key if more than 1 dataset

        if (bodyLines.length > 1) {
          var style = span = '<span class="chartjs-tooltip-key" style="background:' + colors.backgroundColor + ';"></span>';
        }
...
var percentageFooterCallback = function percentageFooterCallback(tooltipItems, data) {
  var label = "";
  var sum = 0;
  var val = 0;
...
var percentageStackedFooterCallback = function percentageStackedFooterCallback(tooltipItems, data) {
  var label = "";
  var sum = 0;
  var val = 0;
  tooltipItems.forEach(function (tooltipItem) {
...
var doughnutLegendCallback = function doughnutLegendCallback(chart) {
...
var percDoughnutLegendCallback = function percDoughnutLegendCallback(chart) {
...
var cardLegendCallback = function cardLegendCallback(chart) {
...
var tabLegendCallback = function tabLegendCallback(chart) {
...
(function ($) {
  $(document).ready(function () {
    ...
    Chart.defaults.scale.gridLines.zeroLineColor = 'rgba(0,0,0,.08)'; // Chart.defaults.bar.categoryPercentage = 1;

    Chart.defaults.bar.scales.xAxes[0].barPercentage = .85;
    Chart.defaults.bar.scales.xAxes[0].categoryPercentage = 1;
    Chart.defaults.global.legend.display = false; // Candlestick styles
    ...
    Chart.defaults.global.elements.point.borderColor = 'rgba(0,0,0,.1)'; // Line styles

    Chart.defaults.global.elements.line.borderColor = 'rgb(0,0,0)'; // Area styles
    ...
    Chart.defaults.global.tooltips = Object.assign(Chart.defaults.global.tooltips, tooltipsOpts); // Set default animations
    ...
    $("#main-toggle-line-chart").closest('.card').find('.card-metrics').on('click', '.card-metric', function (e) {
      e.stopPropagation();
    ...
    delete doughnutTooltip.mode; // Doughnut Chart
    ...
    var table = $('#default-table-example').DataTable({
      ...
      'drawCallback': function drawCallback(settings) {
        var api = this.api(); // Add waves to pagination buttons
